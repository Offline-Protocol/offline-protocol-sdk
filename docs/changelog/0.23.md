...
> [docs/UPGRADING.md §16](../UPGRADING.md#16-replicated-documents-are-available-11-and-in-groups-v0230).
>
...
> [docs/UPGRADING.md §15](../UPGRADING.md#15-the-learned-route-api-is-gone-v0230).
>
...
- **Documents can carry attachments, and a document too large for a frame now
  ...
  replication](../spec/data-sync.md), with frozen conformance vectors
  beside it.
...
- **Documents replicate between peers, over the delivery ladder that was
  ...
  **Changes arriving from a peer are contained rather than trusted.** MLS
  ...
  and [threat model R11](../security/threat-model.md).
...
- **Replicated documents: a second application class on the protocol.** A new
  ...
  *default* provider's account directory, which a custom backend is not inside,
  ...
  [ADR 0018](../adr/0018-data-layer-engine-and-storage-seams.md).
...
- **Replicated documents have a guide, a state machine and examples that run.**
  ...
  [docs/data.md](../data.md) is that document, and it answers the questions
  ...
  [The replication state machine](../state-machines/data-replication.md)
...
- **The gateway contract is specified** in
  [docs/spec/gateway-contract.md](../spec/gateway-contract.md): what a
...
- **Two decisions recorded.**
  [ADR 0016](../adr/0016-gateways-are-provisioned-not-emergent.md): gateways
  ...
  silent partitions.
  [ADR 0017](../adr/0017-nostr-is-a-carrier-not-a-gateway.md): Nostr is a
