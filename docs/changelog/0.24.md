...
> [docs/UPGRADING.md §18](../UPGRADING.md#18-settings-you-already-pass-start-applying-on-ios-v0241).
>
...
> [docs/spec/leaf-provisioning.md](../spec/leaf-provisioning.md) and
...
> on. See [ADR 0023](../adr/0023-a-control-frame-states-when-it-was-made.md)
> and [docs/UPGRADING.md §17](../UPGRADING.md#17-two-refusals-that-are-new-and-one-rotation-you-now-owe-v0240).
>
...
- **A control frame now states when it was made, and a captured one stops
  ...
  **Nothing an application does changes.** Peers negotiate the payload through
  ...
  See [ADR 0023](../adr/0023-a-control-frame-states-when-it-was-made.md).
...
- **`offline-protocol-core` compiles without `std`.** The crate now builds for
  ...
  [ADR 0020](../adr/0020-core-compiles-without-std.md).
...
- **`offline-protocol-sealed`, a new crate holding what both ends of a sealed
  ...
  implementations ([ADR 0021](../adr/0021-a-leaf-node-speaks-mls.md)) and
...
- **What a leaf node owes at pairing is specified** in
  [docs/spec/leaf-provisioning.md](../spec/leaf-provisioning.md). A leaf is
...
- **The leaf profile in [capability
  negotiation](../spec/capability-negotiation.md)**: what a minimal device
...
- **Two threat-model entries for the device class**:
  [A8, the provisioning-time adversary](../security/threat-model.md), who
...
- **`offline-protocol-leaf`: a constrained device that speaks this protocol as
  ...
  [ADR 0021](../adr/0021-a-leaf-node-speaks-mls.md) took and this crate
  implements.
...
- **A leaf node now answers the frames it receives, and a phone stops retrying
  ...
  **For integrators**, two things change on the device. A leaf emits one small
  ...
  [the leaf provisioning spec](../spec/leaf-provisioning.md#what-a-leaf-owes-for-a-frame-it-received).
...
- **The embedded footprint harness measures the shipping crate.** Its leaf
  ...
  The 400 KiB figure in [ADR 0021](../adr/0021-a-leaf-node-speaks-mls.md)
...
- **The Bluetooth LE framing a leaf's firmware must speak is specified** in
  [docs/spec/ble-framing.md](../spec/ble-framing.md). Everything above the
...
- **Why a leaf node speaks MLS through a second implementation is recorded** in
  [ADR 0021](../adr/0021-a-leaf-node-speaks-mls.md), along with the interop
