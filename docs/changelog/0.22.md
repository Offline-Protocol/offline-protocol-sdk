...
>
> Migration notes in
> [UPGRADING §11.3](../UPGRADING.md#113-relay_promoted-and-relay_demoted-can-now-actually-fire-v0220).
...
- **Username discovery and a self-certifying invite payload.** Two ways to
  ...
  **The directory is not authoritative, and the API is shaped so you cannot
  ...
  [docs/spec/username-discovery.md](../spec/username-discovery.md).
...
- **Design documentation restructured; `CLAUDE.md` reduced from 61 KB to 7.6 KB.**
  ...
  [`docs/spec/`](../spec/README.md) is an implementation-independent protocol
  ...
  [`docs/security/threat-model.md`](../security/threat-model.md) states the
  adversary classes, trust boundaries and residual risks plainly;
  [`docs/state-machines/`](../state-machines/README.md) documents delivery and
  acknowledgement, outbox and retries, session lifecycle, group message
  lifecycle and transport lifecycle with diagrams;
  [`docs/adr/`](../adr/README.md) records fifteen decisions that are expensive
  to reverse or easy to undo by accident; and
  [`docs/bridges/`](../bridges/README.md) writes down the Rust-to-Swift,
  ...
  [ADR 0003](../adr/0003-self-certifying-addresses.md) warns breaks
...
- **`CHANGELOG.md` is archived by release series.** The working file was 2,425
  ...
  [`docs/changelog/`](README.md), one file per minor series with
