...
export function GroupDetailScreen({ groupId, groupName, onBack }: GroupDetailScreenProps) {
  ...
  const { send, authenticatedUser, status, groupDetails, getGroupInfo, groupMessages } = useWebSocketRelay();
  const { protocol, isInitialized } = useProtocol();
  const [activeTab, setActiveTab] = useState<Tab>('info');
  const [loading, setLoading] = useState(true);
  const [pendingMessages, setPendingMessages] = useState<GroupMessage[]>([]); // Optimistic messages not yet confirmed
  ...
  // Get messages from context and merge with pending
  const messages: GroupMessage[] = React.useMemo(() => {
    ...
    const converted = contextMessages.map(m => ({
      id: m.id,
      sender: m.sender,
      content: m.content,
    ...
    // Filter out pending messages that are now in context (by content match)
    const pendingNotInContext = pendingMessages.filter(
      pm => !converted.some(cm => cm.content === pm.content && cm.isFromMe)
    );
    return [...converted, ...pendingNotInContext].sort(
      (a, b) => a.timestamp.getTime() - b.timestamp.getTime()
    );
  }, [groupMessages, groupId, pendingMessages]);
  ...
  }, [groupId, usernameInput, protocol, send, status, isInitialized, getGroupInfo]);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const sendMessageJson = await protocol.groupSendMessage(groupId, content);
      ...
      // Add message to pending state immediately (optimistic update)
  ...
  }, [groupId, messageInput, protocol, send, status, authenticatedUser, isInitialized]);
    ...
    return (
      <ScrollView style={styles.tabContent} contentContainerStyle={styles.tabContentContainer}>
        <View style={[styles.infoCard, { backgroundColor: theme.colors.surface }]}>
          <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Group Name</Text>
          <Text style={[styles.infoValue, { color: theme.colors.text }]}>{groupInfo.name}</Text>
        </View>

        <View style={[styles.infoCard, { backgroundColor: theme.colors.surface }]}>
          <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Created By</Text>
        ...
        <View style={[styles.infoCard, { backgroundColor: theme.colors.surface }]}>
          <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Created At</Text>
          <Text style={[styles.infoValue, { color: theme.colors.text }]}>
        ...
        <View style={[styles.infoCard, { backgroundColor: theme.colors.surface }]}>
          <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Members</Text>
          <Text style={[styles.infoValue, { color: theme.colors.text }]}>
  ...
  const renderMembersTab = () => {
    ...
    return (
      <ScrollView style={styles.tabContent} contentContainerStyle={styles.tabContentContainer}>
        {isAdmin && (
          <View style={[styles.addMemberCard, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Add Member</Text>
            <View style={styles.addMemberInputContainer}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor: addingMember ? theme.colors.textSecondary : theme.colors.primary,
                  },
                ]}
              ...
              >
                ...
                {member.role === 'admin' && (
                  <View style={[styles.adminBadge, { backgroundColor: theme.colors.primary }]}>
                    <Text style={[styles.adminBadgeText, { color: theme.colors.textInverse }]}>
                      Admin
                    </Text>
                  </View>
                )}
            ...
            <View
              style={[
                ...
                item.isFromMe
                  ...
                  : [styles.messageBubbleOther, { backgroundColor: theme.colors.surface }],
              ]}
            >
              {!item.isFromMe && (
        ...
        <View style={[styles.chatInputContainer, { backgroundColor: theme.colors.surface }]}>
          ...
          <TouchableOpacity
            style={[
              styles.sendButton,
              {
                backgroundColor: messageInput.trim() ? theme.colors.primary : theme.colors.textSecondary,
              },
            ]}
          ...
          >
            <Icon
              name="send"
              size={20}
              color={messageInput.trim() ? theme.colors.textInverse : theme.colors.textSecondary}
            />
          </TouchableOpacity>
        </View>
      </View>
          ...
          >
            <Text
              style={[
                ...
                {
                  color:
                    activeTab === tab ? theme.colors.primary : theme.colors.textSecondary,
