...
class RecipientInFlightTrackerTest {
    ...
    @Test
    fun unrecordRemovesTheExactEntry() {
        val tracker = RecipientInFlightTracker(ttlMs = 60_000, maxPerRecipient = 32)
        tracker.recordSent("bob", "m1", nowMs = 1_000)
        tracker.recordSent("bob", "m2", nowMs = 2_000)
        ...
        tracker.unrecord("bob", "m1")
        assertEquals(listOf("m2"), tracker.drainRecipient("bob", nowMs = 3_000))

        // No-ops, never throws.
        tracker.unrecord("bob", "gone")
        tracker.unrecord("nobody", "m1")
        tracker.unrecord("", "")
    }
