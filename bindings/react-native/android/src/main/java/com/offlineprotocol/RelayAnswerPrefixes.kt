 ...
 * locally synthesized answer looks like. Passing a non-null actor to
...
object RelayAnswerPrefixes {
    ...
    @JvmStatic
    fun isRelayAnswer(prefix: String): Boolean = prefix in ALL

    /**
     * The actor a synthesized frame may be attributed to: `null` for a relay
     ...
     * actor would be silently dropped as unsigned — a failure no Kotlin or
     * Swift test would otherwise catch, since neither side compiles the other.
     */
    @JvmStatic
    fun attributableActor(prefix: String, actorId: String?): String? =
        if (isRelayAnswer(prefix)) null else actorId
}
