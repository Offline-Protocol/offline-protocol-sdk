...
- **`offline-protocol-exchange` crate + protocol integration + `MeshExchange` FFI** — An open capability exchange layered on service discovery: any node can publish an **attested listing** (a request/response *service* or a content-addressed model *adapter*), discover listings across the mesh with attestation status and a local reputation read, invoke them, and settle metered invocations with dual-signed usage receipts. The wire format is unchanged — a listing is a normal `ServiceDescriptor` carrying a versioned JSON envelope under the reserved `x-op-listing` capabilities key, so exchange-unaware nodes interoperate untouched. Provenance is mandatory: listings are signed by the node's MLS Ed25519 identity key over canonical length-prefixed bytes (`OP-EXCHANGE-LISTING-V1`), and paid invocation / adapter loading refuse anything but a verified attestation. **Priced traffic never rides plaintext**: `invoke_listing` on a priced listing requires a confirmed MLS session, and the settlement control messages (`__XCHG_USAGE__`, `__XCHG_RCPT__`, `__XCHG_RCPT_ACK__`) are signed, TOFU-checked, and dropped rather than sent over an unconfirmed channel. Settlement is prepaid-balance based with two-phase holds (`unit_price × max_units` held before send, debited for actual usage on receipt, released on error/timeout) and eventual clearing through a `SettlementBackend` trait (`MockClearing` ships for CI; the protocol fee applies at settlement with ceiling rounding). Adapter weights move over the media-transfer path and are verified against the attested SHA-256 on arrival — a mismatch discards the bytes (`AdapterPullRejected`) and the `AdapterRuntime` load gate re-verifies before any load. Eight new events (`ListingDiscovered`, `ExchangeReceiptIssued`/`Received`/`Acknowledged`, `ExchangeBalanceChanged`, `ExchangeInvocationFailed`, `AdapterPullCompleted`/`Rejected`) flow through the existing event/telemetry/scrubbing machinery, durable exchange state (ledger, receipts, reputation) persists via `MlsStorage`, and the full publish → discover → fund → invoke → receipt → settle loop is covered by integration tests over the in-memory transport, including the plaintext-refusal and hash-mismatch cases. Exposed to Swift/Kotlin/React Native as `MeshExchange` mirroring the `MeshServices` pattern. See `docs/capability-exchange.md`.
- **`offline-protocol-node`: headless mesh node with a localhost control API** — A daemon that runs a full node (MLS identity, capability exchange, transports) on servers/Pis/laptops and exposes a localhost HTTP API mirroring the Capability Exchange `MeshBridge` interface: discover/list listings, blocking invoke (waits for the mesh response + signed receipt), hash-verified adapter pulls, mesh-ledger balance/credit, pending-receipt export and mark-settled, and attested publish endpoints. Binds `127.0.0.1` with optional bearer auth (`NODE_API_TOKEN`); MLS key material and exchange state persist via a new file-backed `MlsStorage` (atomic writes, hex-encoded key ids, `0700` data dir). Primary consumer: the Capability Exchange MCP server's `node` mode. See `docs/headless-node.md`.
- **Multi-hop discovery-response docs corrected** — `MeshServices` discovery responses *are* relayed hop-by-hop back to the originator (each node responds to the peer it received the query from and intermediates forward toward the originator); the stale "one hop back" limitation note on `MeshServices` and `discover_services` has been removed.
