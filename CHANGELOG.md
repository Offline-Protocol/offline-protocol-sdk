...
- **Group messages that outrun their Welcome (or a commit) are buffered instead of permanently lost.** The Welcome and the first group message are sent back-to-back and can arrive out of order across transports — but a group application message that failed MLS decryption was dropped on the spot, and because its ID was already recorded for replay protection, every redelivery (outbox retry, second transport, relay replay) was rejected as a duplicate: the first message(s) after a group invite could never be delivered. Application messages now get the same treatment out-of-order commits already receive: a decrypt failure caused by lagging local group state (`GroupNotFound` — Welcome not yet processed — or an epoch-behind `Decryption` failure) buffers the ciphertext (16 per group, TTL matched to the 5-minute replay-dedup window so the buffered copy outlives the dedup entry that blocks redelivery), and the buffer is drained in arrival order after a successful Welcome join or commit for that group. Applies to both the mesh (`__GRP_MLS_MSG__`) and relay (`__GROUP_MSG__`) inbound paths, whose decrypt-and-classify logic is now shared; permanent failures (bad payloads, sender-identity mismatches) are still dropped immediately, and buffers are cleared on group leave/removal. Commits get the same repair: `GroupNotFound` was previously classified as a permanent commit failure, so a commit that outran the Welcome was dropped outright — it is now buffered like any other out-of-order commit, and a successful Welcome join drains buffered commits first (advancing the epoch) and then buffered messages, so the invite → membership-change → first-message sequence delivers regardless of arrival order. Because `GroupNotFound` buffering is by definition pre-authentication, both buffers are now also globally bounded across groups (messages: 256 entries / 8 MiB of ciphertext / 32 distinct group IDs; commits: 64 entries / 4 MiB / 16 distinct group IDs) — per-group caps alone left total retention open-ended across attacker-chosen group IDs, and the distinct-group cap also bounds map-key memory (group IDs are wire-supplied, up to 4 KiB each). At entry/byte capacity the oldest entry of the *largest* per-group buffer is evicted, so a flood that concentrates in a few group IDs is evicted before honest welcome-race entries (few per group); freeing a *group slot* at the distinct-group cap instead evicts the single largest per-group buffer wholesale — emptying some group is the only way to free a slot, and doing it one entry at a time would round-robin across level-sized buffers and purge nearly the whole map before any group emptied. A spread flood (one entry per fabricated group ID) levels the buffer sizes and degrades eviction to globally-oldest within the group cap — to keep even that from causing permanent loss, any buffered entry dropped undelivered (cap eviction or drain-time TTL expiry) releases its replay-dedup ID, so a sender-side redelivery is accepted fresh instead of being rejected as a duplicate of a copy that no longer exists; this applies to buffered commits exactly as to buffered messages (each pending commit records its wire message ID for this). An entry that alone exceeds the byte budget is dropped outright instead of purging the buffer; buffered commits are also cleared on group leave/removal alongside buffered messages. A commit or proposal that arrives *on the application-message channel* (MLS consumes it and returns no plaintext) now also counts as a state advance: it triggers the same buffered-commit and buffered-message drains as a commit-channel success — including mid-drain, where a consumed riding commit re-runs the drain pass so entries buffered ahead of it in arrival order still deliver.
