...
- **`resetTofuForPeer` now drops the stale MLS session atomically** — `OfflineProtocol::reset_tofu_for_peer` (exposed as `resetTofuForPeer`) previously only un-pinned the TOFU key, leaving the now-dead 1:1 MLS session in place so callers had to remember to delete it or the next `establish_secure_session` would no-op against stale credentials. It now also drops the session (best-effort — no session or uninitialized MLS is a harmless no-op) in the same call, since accepting a peer's new identity makes the old session dead by definition. Behavior is unchanged when no TOFU entry exists (still returns `false`, still idempotent).
