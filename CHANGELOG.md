...
- **`uninstallTelemetrySink()`** — Detaches the installed sink in a single call: replaces the core-side sink with a no-op (future emissions are discarded with zero overhead) and drains the pull queue so a subsequent `installTelemetrySink(...)` starts with an empty queue. Idempotent. Previously the only way to stop telemetry was to install a user-authored no-op sink, which was source-compatible but asymmetric with install.
- **`installTelemetrySink(config, listener?)` accepts an optional listener** — Registering the listener synchronously before the native install is dispatched closes the window where records emitted between the bridge resolving and the next JS microtask would fan out to an empty listener set and be dropped on the push channel. Standalone `onTelemetry(...)` still works; pass the listener here if you want the race closed.
