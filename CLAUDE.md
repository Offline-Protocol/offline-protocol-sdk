...
- **Group send paths (per-member is the default, broadcast is opt-in)**: `send_group_message_inner` (`group_mesh.rs`) either fans the MLS ciphertext out as one `__GRP_MLS_MSG__` `SendMessage` per member — which inherits the whole DM ladder (outbox, ACK/retry, relay write-ack, offline push with ciphertext, park/probe/flush, receiver deferred-ACK) — or sends a single `__GRP_RELAY_BCAST__` hint the bridge translates into a relay-native `SendGroupMessage`. The broadcast is O(1) on sender uplink but has **no per-recipient delivery contract** (relay fan-out is fire-and-forget: no presence check, no push, no persistence, "sent" answered before delivery), and a missed group message is undetectable because MLS application messages don't advance the epoch. So it is gated on `GroupConfig::relay_broadcast_enabled` (**default false**) *and* `relay_synced` *and* a live `is_internet_available()` re-check (a stale sync flag must not route a broadcast into the mesh). `GroupConfig::relay_enabled` is separate and stays **on** — it gates *registration*, which invite links resolve against. Per-member costs O(N) frames; the relay rate limiter (30 burst / 10 per sec, disconnect after 10 consecutive violations) makes that comfortable to ~30 members, self-healing to ~40.
- **Relay hint frames (`__GRP_RELAY_REG__`, `__GRP_RELAY_BCAST__`)**: self-addressed frames the *bridge* (not the relay) recognizes via `internet_control_op` and **replaces** with relay-native frames. Because the frame is replaced, no ACK can ever return for it — so both go out via `send_relay_hint_message` (`protocol/send.rs`), which is `requires_ack = false` (no outbox entry, no pending ACK, no retry entry) and **pinned to `TransportType::Internet`**, propagating transport errors to the caller instead of deferring. Both properties are load-bearing: on the ACK ladder an un-ACKable frame is retransmitted 10× over ~800s (each resend a full extra relay fan-out under a fresh relay-minted id that receiver dedup misses), ending in a `MessageFailed` for an id the app never saw plus a DORS-degrading delivery failure; and under ordinary DORS routing (which demotes Internet by `INTERNET_FALLBACK_DEMOTION`) a self-addressed frame is *swallowed* by Wi-Fi Direct or Reticulum, which enqueue unconditionally and return `Ok` — making `try_relay_broadcast` report success and skip the per-member fallback, delivering to nobody. Retry policy lives at the application layer instead: `RelayRegisterPending` (30s × 3) for registration, per-member fallback for broadcast. Coverage: `test_relay_broadcast_frame_is_unacked_and_not_retried`, `test_relay_registration_frame_is_unacked_and_not_retried`, `test_relay_hint_frames_pin_to_internet_not_mesh`, `test_relay_broadcast_falls_back_when_internet_unavailable`, `test_relay_broadcast_disabled_by_default_uses_per_member_fanout`.
