...
use offline_protocol_leaf::{LeafDevice, LeafError, LeafEvent, LeafStore, MemoryStore, StoreError};
use offline_protocol_mls::{storage::InMemoryStorage, MlsManager, MlsStorage};
use offline_protocol_sealed::{
    control_signing_payload, derive_address, prefixes, EncryptedMessage, KeyPackagePayload,
...
fn pair(phone: &Phone, device: &LeafDevice) -> String {
...
fn a_phone_and_a_device_pair_and_talk_both_ways() {
    ...
    let device_address = pair(&phone, &device);
...
fn a_driven_rekey_reaches_the_device_as_a_session_reset() {
    let phone = new_phone();
    let store = Arc::new(MemoryStore::new());
    let device = device(Arc::clone(&store) as Arc<dyn LeafStore>);
    let device_address = pair(&phone, &device);
...
fn an_unsigned_control_frame_is_refused() {
    let phone = new_phone();
    let device = device(Arc::new(MemoryStore::new()));
...
fn state_survives_a_power_cycle() {
    ...
    let device_address = {
        let device = device(Arc::clone(&store));
        pair(&phone, &device)
    };
    ...
    let revived = LeafDevice::resume(Arc::clone(&store), APP_ID).expect("device resumes");
    assert_eq!(
