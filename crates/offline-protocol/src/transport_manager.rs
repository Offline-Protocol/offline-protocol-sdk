...
impl TransportManager {
    ...
    /// `nostr_enabled` defaults off) or when the peer advertised nothing. The
    /// bound on the underlying map lives in the transport.
    pub fn mark_peer_nostr_pubkey(&mut self, peer_id: &str, pubkey_hex: Option<&str>) {
        ...
        {
            nostr.set_peer_nostr_pubkey(peer_id, pubkey_hex);
        }
    }
